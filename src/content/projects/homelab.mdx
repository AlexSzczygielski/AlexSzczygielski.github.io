...
For now the network runs entirely on the ISP-issued router. A managed switch (for VLANs / traffic segmentation) in tandem with router capable of running pfsense or opensense OS is the next step. As ISP's router does not allow bridging, I will be looking into the `double NAT` configuration. 

It is impossible to build the entire homelab at once, thus network devices are waiting for their turn. Right now if there ever was an urgent need I could enforce some isolation in the software layer instead - using docker networking.
